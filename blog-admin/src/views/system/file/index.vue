<template>
    <div class="app-container">
                ...
                <el-table-column label="文件内容" align="center" prop="filename" >
                    <template #default="scope">
                        <div v-if="isImage(scope.row.ext)">
                            <el-image :initial-index="0" :src="scope.row.url"
                                style="width: 50px; height: 50px"  @click="handlePreview(scope.row)"/>
                        </div>
                        <div v-else>
        ...
        <!-- 图片预览弹窗 -->
        <el-image-viewer v-if="openPreview && isImage(currentPreviewFile.ext)" @close="closeViewer" :url-list="previewList" />

        <!-- 视频预览弹窗 -->
        <el-dialog v-if="openPreview && isVideo(currentPreviewFile.ext)" title="视频预览" v-model="openPreview" width="600px" append-to-body>
        ...
        <!-- 文档预览弹窗 -->
        <el-dialog v-if="openPreview && !(isImage(currentPreviewFile.ext) || isVideo(currentPreviewFile.ext))" 
                   :title="`文件预览: ${currentPreviewFile.filename}`" 
                   v-model="openPreview" 
                   width="80%" 
                   append-to-body>
            <div class="file-preview-container">
                <iframe v-if="isPreviewableDocument(currentPreviewFile.ext)" 
