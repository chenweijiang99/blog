<template>
  <div class="article-page" v-loading="loading">
    ...
    <div class="floating-action-bar" :style="{ left: actionBarLeft }">
      ...
      <div class="action-item reward-item" v-if="$store.state.webSiteInfo.open_admiration === 1">
        <div class="action-button">
 ...
 */
      ...
      try {
        ...
        try {
          ...
          if (successful) {
            ...
            // 如果两种方法都失败，显示URL并提示用户手动复制
            this.showFallbackCopyDialog()
          }
        } catch (fallbackErr) {
          // 如果两种方法都失败，显示URL并提示用户手动复制
          this.showFallbackCopyDialog()
    ...
    showFallbackCopyDialog() {
      ...
      this.$alert(
        `<div style="word-break: break-all; user-select: all;">${url}</div>`,
        '复制链接',
        {
 ...
 */
    removeExistingSummary() {
      // 查找所有可能包含摘要的details元素
      const detailsElements = document.querySelectorAll('.article-content details');
      detailsElements.forEach(details => {
        const summary = details.querySelector('summary');
        if (summary) {
          // 检查是否为代码块相关的details元素
          const isCodeBlock = details.classList.contains('md-editor-code');

          if (isCodeBlock) {
            // 对于代码块类型的details，保留pre元素
            const preElements = details.querySelectorAll('pre');
            if (preElements.length > 0) {
              // 将pre元素移动到details外部
              const parent = details.parentNode;
              preElements.forEach(pre => {
                parent.insertBefore(pre.cloneNode(true), details);
              });
              // 移除整个details元素
              details.remove();
            }
          } else {
            // 对于其他类型的details，检查summary内容是否包含摘要相关关键词
            const summaryText = summary.textContent.toLowerCase();
            if (summaryText.includes('摘要') || summaryText.includes('summary')) {
              // 保留pre元素并移除details
              const preElements = details.querySelectorAll('pre');
              if (preElements.length > 0) {
                const parent = details.parentNode;
      ...
      // 移除其他可能的summary元素
      const summaryElements = document.querySelectorAll('.article-content .summary');
